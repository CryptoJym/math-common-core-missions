...
class TestBradyPersonalizationTables:
    ...
    def test_brady_practice_attempts_table_exists(self):
        resp = supabase_rest_get("brady_practice_attempts", {"select": "id", "limit": "1"})
        assert resp.status_code in (200, 401, 403), \
            f"brady_practice_attempts table query returned unexpected status {resp.status_code}"

    def test_brady_practice_attempts_rls_blocks_anon_reads(self):
        resp = supabase_rest_get("brady_practice_attempts", {"select": "*"})
        if resp.status_code == 200:
            assert resp.json() == [], "RLS should prevent anonymous access to brady_practice_attempts"

    def test_brady_practice_drafts_table_exists(self):
        resp = supabase_rest_get("brady_practice_drafts", {"select": "id", "limit": "1"})
        assert resp.status_code in (200, 401, 403), \
            f"brady_practice_drafts table query returned unexpected status {resp.status_code}"

    def test_brady_practice_drafts_rls_blocks_anon_reads(self):
        resp = supabase_rest_get("brady_practice_drafts", {"select": "*"})
        if resp.status_code == 200:
    ...
    def test_brady_ai_learner_profile_table_exists(self):
        ...
        assert resp.status_code in (200, 401, 403), \
            f"brady_ai_learner_profile table query returned unexpected status {resp.status_code}"

    def test_brady_ai_learner_profile_rls_blocks_anon_reads(self):
        resp = supabase_rest_get("brady_ai_learner_profile", {"select": "*"})
        if resp.status_code == 200:
            assert resp.json() == [], "RLS should prevent anonymous access to brady_ai_learner_profile"

    def test_brady_artifacts_table_exists(self):
        resp = supabase_rest_get("brady_artifacts", {"select": "id", "limit": "1"})
        assert resp.status_code in (200, 401, 403), \
            f"brady_artifacts table query returned unexpected status {resp.status_code}"

    def test_brady_artifacts_rls_blocks_anon_reads(self):
        resp = supabase_rest_get("brady_artifacts", {"select": "*"})
        if resp.status_code == 200:
            assert resp.json() == [], "RLS should prevent anonymous access to brady_artifacts"

    def test_brady_ai_reviews_table_exists(self):
        resp = supabase_rest_get("brady_ai_reviews", {"select": "id", "limit": "1"})
        assert resp.status_code in (200, 401, 403), \
            f"brady_ai_reviews table query returned unexpected status {resp.status_code}"

    def test_brady_ai_reviews_rls_blocks_anon_reads(self):
        resp = supabase_rest_get("brady_ai_reviews", {"select": "*"})
        if resp.status_code == 200:
            assert resp.json() == [], "RLS should prevent anonymous access to brady_ai_reviews"
